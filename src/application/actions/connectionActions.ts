...
function rpcClient(): TrialRpcClient {
  return createServiceClient() as unknown as TrialRpcClient
}
...
export async function createMomentAction(input: unknown): Promise<ActionResult<{ id: string }>> {
  try {
    ...
    const { data, error } = await rpcClient().rpc<string>('create_moment_for_trial', {
...
export async function cancelMomentAction(momentId: string, surrogacyId: string): Promise<ActionResult> {
  try {
    ...
    const { error } = await rpcClient().rpc<null>('cancel_moment_for_trial', {
      p_moment_id: id,
      p_actor_id: actor.id,
    })
...
export async function completeMomentAction(
  ...
  try {
    ...
    const { data, error } = await rpcClient().rpc<string>('complete_moment_for_trial', {
      p_moment_id: id,
      p_actor_id: actor.id,
...
export async function submitFeedbackAction(input: unknown): Promise<ActionResult<{ id: string }>> {
  try {
    ...
    const { data, error } = await rpcClient().rpc<string>('submit_feedback_for_trial', {
