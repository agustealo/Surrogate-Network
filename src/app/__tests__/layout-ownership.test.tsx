...
jest.mock('@/infrastructure/supabase/server', () => ({
  createClient: jest.fn(async () => ({
    ...
    from: jest.fn(() => ({ select: jest.fn(() => ({ eq: jest.fn(() => ({ single: jest.fn(async () => ({ data: { is_admin: true }, error: null })) })) })) })),
  })),
}));
jest.mock('@/components/public/PublicNavigation', () => ({ PublicNavigation: () => <div>PublicNavigation</div> }));
jest.mock('@/components/public/PublicFooter', () => ({ PublicFooter: () => <div>PublicFooter</div> }));
jest.mock('@/components/member/MemberHeader', () => ({ MemberHeader: () => <div>MemberHeader</div> }));
jest.mock('@/components/member/MemberNavigation', () => ({ MemberNavigation: () => <div>MemberNavigation</div> }));
jest.mock('@/components/member/MobileNavigation', () => ({ MobileNavigation: () => <div>MobileNavigation</div> }));
jest.mock('@/components/admin/AdminHeader', () => ({ AdminHeader: () => <div>AdminHeader</div> }));
jest.mock('@/components/admin/AdminNavigation', () => ({ AdminNavigation: () => <div>AdminNavigation</div> }));
jest.mock('@/components/ui/toaster', () => ({ Toaster: () => <div>Toaster</div> }));
...
describe('layout ownership', () => {
  ...
  it('renders the authenticated member shell without public navigation', async () => {
  ...
  it('renders the authorized admin shell without public navigation', async () => {
