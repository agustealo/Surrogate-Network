...
export default async function RewardsPage() {
  ...
  return (
    <div className="container mx-auto max-w-5xl px-4 py-6">
      ...
      <Card className="mt-6"><CardHeader><CardTitle>Recent XP activity</CardTitle></CardHeader><CardContent>{xpRows?.length ? <div className="divide-y">{xpRows.map((row) => <div key={row.id} className="flex items-start justify-between py-3"><div><p className="font-medium">{row.description}</p><p className="text-xs text-muted-foreground">{row.source} · {new Date(row.created_at).toLocaleDateString()}</p></div><span className="font-mono">+{row.amount}</span></div>)}</div> : <p className="text-muted-foreground">No XP activity yet.</p>}</CardContent></Card>
    </div>
  );
}
